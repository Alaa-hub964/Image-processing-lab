...
// ── BIDIRECTIONAL MATCHING ADDED HERE TO FIX "BAD RESULTS" ──
function matchKps(kps1, ds1, kps2, ds2, ratio = 0.82) {
  const M = [];
  for (let i = 0; i < kps1.length; i++) {
    ...
    if (d1 < ratio * ratio * d2 && bj >= 0) {
      // Cross-check: ensures point B also thinks point A is its best match
      let r_d1 = Infinity, r_d2 = Infinity, r_bi = -1;
      for (let k = 0; k < kps1.length; k++) {
        let ssd = 0; const a = ds1[k], b = ds2[bj];
        for (let m = 0; m < a.length; m++) { const v = a[m] - b[m]; ssd += v * v; }
        if (ssd < r_d1) { r_d2 = r_d1; r_d1 = ssd; r_bi = k; } else if (ssd < r_d2) r_d2 = ssd;
      }
      if (r_bi === i) M.push({ i, j: bj, d: d1 });
    }
  }
  M.sort((a, b) => a.d - b.d);
  return M;
...
// ----------------------------------------------------------
// REGISTRATION PANEL (UNIFIED BLOCK)
// ----------------------------------------------------------
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "60vh", gap: 16, color: "rgba(255,255,255,0.5)" }}>
          ...
          <button onClick={() => this.setState({ hasError: false, error: null })}
            style={{
              background: "rgba(247,37,133,0.1)", border: "1px solid #f72585", color: "#f72585",
...
function RegistrationPanel({ color, activeTopic }) {
  ...
  const runAll = () => {
    ...
    setTimeout(() => {
      try {
        ...
        // 1. Detect
        const kps1 = detectHarrisCorners(g1, img1.W, img1.H, 400);
        const kps2 = detectHarrisCorners(g2, img2.W, img2.H, 400);
        // 2. Describe & match (Bidirectional enabled)
        const ds1 = kps1.map(({ x, y }) => patchDescriptor(g1, img1.W, img1.H, x, y));
        const ds2 = kps2.map(({ x, y }) => patchDescriptor(g2, img2.W, img2.H, x, y));
        const matches = matchKps(kps1, ds1, kps2, ds2, 0.82);
        // 3. RANSAC (Tighter threshold)
        const p1 = matches.map(({ i }) => [kps1[i].x, kps1[i].y]);
        const p2 = matches.map(({ j }) => [kps2[j].x, kps2[j].y]);
        const res = matches.length >= 4 ? doRansac(p1, p2, 4, 1500) : null;
        // 4. Warp
        ...
        if (res) {
          ...
          for (let p = 0; p < dW * dH; p++) {
            ...
            if (warpedData[oi + 3] > 0) {
              // Overlapping moving image and warped reference
  ...
  return (
    <div style={{ padding: 14, overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {/* Upload panels (Hidden once computed to save space) */}
      ...
      <div className={`mob-overlay${mobTab === 'modules' ? ' open' : ''}`}>
        ...
        {MODULES.map(mod => (
          <button key={mod.id} onClick={() => { selMod(mod); setMobTab('ops'); }}
            style={{
              width: "100%", background: activeMod.id === mod.id ? "rgba(255,255,255,0.07)" : "transparent",
        ...
        {Object.entries(activeMod.theory || {}).map(([k, v]) => (
          <div key={k} style={{ marginBottom: 8 }}>
            <div onClick={() => setTheory(theory === k ? null : k)}
              style={{
                cursor: "pointer", padding: "10px 12px", background: "rgba(255,255,255,0.03)",
