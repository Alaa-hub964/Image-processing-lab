...
const MODULES = [
  ...
  { id:"registration",icon:"🎯", label:"Geometric Transforms", color:"#06d6a0",
    topics:["Translation","Rotation","Scaling","Shear","Affine Transform","Projective Warp","Flip H","Flip V","Bilinear Interp"],
    theory:{
      "Translation":"T(tx,ty): shifts image by (tx,ty) pixels. x'=x+tx, y'=y+ty. Simplest spatial transform — 2 degrees of freedom.",
      "Rotation":"Rotates image by angle θ around center (cx,cy). x'=cx+(x-cx)cosθ-(y-cy)sinθ, y'=cy+(x-cx)sinθ+(y-cy)cosθ. Preserves distances and angles.",
      "Scaling":"Scales image by factor s around center. x'=cx+(x-cx)/s, y'=cy+(y-cy)/s. s>1 zooms in, s<1 zooms out. Preserves shape.",
      "Shear":"Shear transform: x'=x+k·y, y'=y. Slants the image along x-axis. Common in text skew correction and synthetic data augmentation.",
      "Affine Transform":"6 DOF linear mapping: x'=a·x+b·y+tx, y'=c·x+d·y+ty. Combines translation, rotation, scale and shear. Preserves parallel lines and ratios of distances.",
      "Projective Warp":"8 DOF perspective transform (homography): maps plane to plane. Defined by 3×3 matrix H. Does NOT preserve parallelism — models camera perspective changes.",
      "Flip H":"Horizontal mirror: x'=W-1-x, y'=y. Equivalent to reflection about vertical axis. Used for data augmentation and symmetry analysis.",
      "Flip V":"Vertical mirror: x'=x, y'=H-1-y. Equivalent to reflection about horizontal axis. Useful for upside-down image correction.",
      "Bilinear Interp":"Weighted average of 4 neighboring pixels for sub-pixel positions: f(x,y)=(1-dx)(1-dy)·f00 + dx(1-dy)·f10 + (1-dx)dy·f01 + dx·dy·f11. Produces smooth warping results."
    }},
